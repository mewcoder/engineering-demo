...
function decodeNamedCharacterReference(value) {
  return own$a.call(characterEntities, value) ? characterEntities[value] : false
}
...
function extension(combined, extension) {
  ...
  for (key in extension) {
    if (own$9.call(extension, key)) {
      ...
      const maybe = own$9.call(combined, key) ? combined[key] : undefined;
      /* c8 ignore next */
 ...
 */
function blockquote$1(node, _, context, safeOptions) {
 ...
 * @param {Emphasis} node
 */
function emphasis$1(node, _, context, safeOptions) {
 ...
 * @param {Heading} node
 */
function heading$1(node, _, context, safeOptions) {
 ...
 */
function imageReference$1(node, _, context, safeOptions) {
 ...
 * @param {Link} node
 */
function link$1(node, _, context, safeOptions) {
  const quote = checkQuote(context);
  const suffix = quote === '"' ? 'Quote' : 'Apostrophe';
  const tracker = track(safeOptions);
  /** @type {Exit} */
 ...
 */
function linkReference$1(node, _, context, safeOptions) {
  const type = node.referenceType;
 ...
 * @param {List} node
 */
function list$1(node, parent, context, safeOptions) {
 ...
 * @param {ListItem} node
 */
function listItem$1(node, parent, context, safeOptions) {
 ...
 * @param {Root} node
 */
function root$1(node, _, context, safeOptions) {
 ...
 * @param {Strong} node
 */
function strong$1(node, _, context, safeOptions) {
